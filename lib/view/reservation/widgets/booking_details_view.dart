...
class _BookingDetailsViewState extends State<BookingDetailsView> {
  ...
  void initState() {
    ...
    super.initState();
    reservationController.subTotalPrice = 0.0;
  ...
  uploadAllBookingDetails() {
    ...
    reservationController.bookRestaurantDetailsApiCall(
      ...
      if (value) {
        ...
        for (int i = 0;
            i < reservationController.reservationOrderMoreMenu.length;
            i++) {
          for (int j = 0;
              j <
                  reservationController
                      .reservationOrderMoreMenu[i].menu!.length;
              j++) {
            /// in previous added quantity , new added quantity is deleted
            reservationController.reservationOrderMoreMenu[i].menu![j]
                .quantity = reservationController
                    .reservationOrderMoreMenu[i].menu![j].quantity! -
                reservationController
                    .reservationOrderMoreMenu[i].menu![j].newAddedQuantity!;

            /// newAdded quantity is made zero
            reservationController
                .reservationOrderMoreMenu[i].menu![j].newAddedQuantity = 0;
          }
        }

        /// subTOTAL PRICE TO BE CLEAR
        reservationController.subTotalPrice = 0.0;
      }
    });
  }
  ...
  loadToGoMenu() {
    toGoReservationController.toGoMenuIsLoading.value = true;
    toGoReservationController.toGoMenuApiCall(body: {
      "RestaurantId": data[0]["RestaurantId"],
      "isOfferItem": true,
    }).then((value) {
      toGoReservationController.toGoMenuIsLoading.value = false;
    });
  }
                            ...
                            children: [
                              ...
                              Visibility(
                                visible: widget.callFrom == 'Previous'
                                    ...
                                    : true,
                                child: ToGoView(
                                  restaurantId: reservationController
                                      .bookRestaurantDetails!
                                      .bookinglistresponse
                                      .restaurantId,
                                  restaurantPic: reservationController
